...
public class InvestorController {
    ...
    public ResponseEntity<Void> createInvestor(@PathVariable String username, @RequestBody InvestorCreate investor) {
        ...
        if (!investor.getInvestorData().toInvestor().isValid()) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<Void> updateInvestor(@PathVariable String username, @RequestBody Investor investor) {
        if (!Objects.equals(investor.getUsername(), username)) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        //if (!investor.isValid()) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        return service.updateOne(investor) ? new ResponseEntity<>(HttpStatus.OK) : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
